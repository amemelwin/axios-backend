...
public class CommonUtils {
	...
	public static boolean isValidUserRole(String userRole) {
		return userRole.matches(CommonData.USER_ROLE_REGEX_PATTERN);
	}

	// CHECK ORDER STATUS
	public static boolean isValidOrderStatus(String orderStatus) {
		return orderStatus.matches(CommonData.ORDER_STATUS_REGEX_PATTERN);
	}

	// CHECK JP POSTAL CODE
	public static boolean isValidPostCode(String postCode) {
		return postCode.matches(CommonData.JAPANESE_POSTAL_CODE_REGEX_PATTERN);
	}

	// CHECK TRANSACTION TYPE
	public static boolean isValidTransactionType(String tranType) {
		return tranType.matches(CommonData.TRANSACTION_TYPE_REGEX_PATTERN);
	}

	// CHECK TRANSACTION STATUS
	public static boolean isValidTransactionStatus(String tranStatus) {
		return tranStatus.matches(CommonData.TRANSACTION_STATUS_REGEX_PATTERN);
	}

	// CHECK TRANSACTION STATUS
	public static boolean isValidProductStatus(String prodStatus) {
		return prodStatus.matches(CommonData.PRODUCT_STATUS_REGEX_PATTERN);
	}

	// アルファベットのみ[Abcddd]
	public static boolean isValidAlphabet(String alphabets) {
		return alphabets.matches(CommonData.REGEX_ALPHABET);
	}

	// 半角英数字のみ[Abcddd2023]
	public static boolean isValidHanMoji(String hanmoji) {
		return hanmoji.matches(CommonData.REGEX_ALPHANUM);
	}
